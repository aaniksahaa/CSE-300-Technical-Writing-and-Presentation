...
\maketitle
	...
	\begin{frame}{Huffman's Idea}
		...
		\begin{block}{The Observation}
			...
			\begin{itemize}
				...
				\item Assigning fewer bits for characters that are more frequent decreases the average length of the encoded message
				
				\item The paths from the root to the leaves of a tree always correspond to a unique prefix-free-encoding
	...
	\begin{frame}{Huffman's Idea}
		...
		\begin{alertblock}{A Greedy Algorithm}
			...
			\begin{enumerate}
				\item Maintain a min-heap of nodes with their relative frequencies as weights. For instance, if the relative appearance of a character is 17\%, we assign the weight of that node to be 0.17
				
				\item Pop two nodes with minimum weights from the min-heap. 
				
				\item Merge the nodes into a single node with the weight as the summation of the previous nodes.
				
				\item Push the newly created node back into the priority queue
				
				\item Repeat from Step 2 until the priority queue contains a single node, the root of the tree.
	...
	\begin{frame}{Why Greedy Technique Works in Huffman Coding}
		...
		\begin{block}{The Intuition Behind}
			\begin{itemize}
				\item The greedy approach consistently selects the least frequent symbols and places them at the deepest position of the tree, ensuring that longer codes are assigned to less probable symbols and vice versa.
				
				\item The construction of Huffman codes follows a bottom-up greedy approach, where the optimal solution is built iteratively.
			\end{itemize}
		\end{block}
	\end{frame}
	
	\begin{frame}{The Huffman Tree}
		\begin{block}{Huffman Tree}
		...
		\end{block}
		\begin{alertblock}{Properties of Huffman Tree}
			\begin{itemize}
...
\end{frame}
	...
	\begin{frame}{How to Encode?}
		...
		\begin{block}{Encoding is easy, isn't it?}
			...
			\textbf{Idea}: We may just look up the binary code for each of the letters and append them one by one to the resulting encoding.
	...
	\begin{frame}{Efficiency of Encoding}
		...
		\]
		Here $p_i$ denotes the probability of $i$th symbol. For our word "DECODE", we calculate entropy as follows-
		$$
		\begin{align*}
